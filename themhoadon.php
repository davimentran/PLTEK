    ...
    if ($affect) {
     ...
     /*]]>*/
                            ...
                            //die();
                                ...
                                if($thanhtoan_id!=$info_hoadon["thanhtoan_id"])
                                {
                                  $ngay_update  = date('d');
                                  $thang_update = date('m');
                                  $nam_update   = date('y');
...
<tbody>
   <tr>
      <td colspan="2" class="boxGrey2">
        ...
        if (isset ($_GET["edit"]) && $_GET["edit"] != '') {
        ?>
         <?php if($function_permission["is_edit"]){?>
...
<?php
      ...
      $rst_group=$dbf->getDynamic("city_vietnam","status=1","title asc");
              ...
              if($toal_group >0)
              {
                while($rows_group = $dbf->nextdata($rst_group))
                {
                  ...
                  echo '<option value="">'.$title_group.'</option>';
      ...
      if ($toal_khachhang > 0) {
        while ($rows_khachhang = $dbf->nextdata($rst_khachhang)) {
          ...
          echo '<option ' . (($id_khachhang == $khachang_id) ? "selected" : "") . ' value="' . $id_khachhang . '|o0o|' . $title_khachhang . '|o0o|' . $mst_khachhang . '|o0o|' . $address_khachhang . '">=>' . $title_khachhang . '' . (($total_no > 0) ? " => Đang nợ: " . $total_no . " hóa đơn (" . $utl->format($tienno_k) . ")" : "") . '' . (($total_datcoc > 0) ? " => Đặt cọc: " . $total_datcoc . " hóa đơn :(" . $utl->format($tiendatcoc_k) . ")" : "") . '</option>';
        }
      }
